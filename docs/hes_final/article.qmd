...
Overall, the results suggest that it is possible to predict upcoming conflict escalation from text embeddings of news articles. 
Different data sampling and modeling approaches produce comparable results based on common metrics. Transformer-based model generalizes better across geographies while statistical model provides a size/cost of deployment advantage.
