...
Our second approach used a pair of transformer models to extract information directly from the GDELT embedding corpus.[@vaswani2017attention] Architecturally, both models are encoder-only transformers, but for simplicity we will refer to the first of these models as “the encoder” and the second as the “the classifier”.

Prior to training, the GDELT dataset was split into one $512 x n$ matrix for each country and month where n is the number of articles in the dataset published in that month related to that country. Each of these matrices was then split into several random subsamples of $512 x 50$. Samples for countries in which $n < 50$ were padded, and multiple samples were taken for countries in which $n > 50$. This resampling dramatically increased the number of training samples by acting as though the same month had occurred multiple times while still maintaining uniqueness of each sample. Simultaneously, limiting the size of each sample to 50 articles reduced the dimensionality of each sample, reducing the risk of overfitting.

The encoder was then trained using these samples, with the time-shifted spike metric as its target. Once training was complete, the final layer of the encoder was removed, exposing the penultimate layer of 16 dense neurons, and its weights were frozen. The encoder was then used to scan each of the original $512 x n$ matrices, resulting in a length-16 vector for each $512 x 50$ subsample. This reduced the dimensions of each sample to $16 x (n/50)$.

The classifier was then trained on this new, distilled dataset without resampling. The same time-shifted spike metric was again used as the training target.  The reduced dimensionality of this dataset was sufficient to offset the risk of overfitting. 
