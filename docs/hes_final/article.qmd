...
We evaluated the models against a set of common performance metrics (see @tbl-best-max-metrics, @tbl-best-min-metrics, @fig-roc, @fig-prc). Despite conceptual differences in dataset creation and modeling techniques, all three of the described above approaches produced comparative results. Each of the models outperformed the other two in a subset of these metrics. This suggests that the choice of the model architecture for this task would depend on specific application goals. We assume that UNICEF decision makers would appreciate a tool that flags all potential rising conflicts even if sometimes it has false positives. At the same time, if the false positive rate is too high the signal would become too noisy for the tool to be useful. Given these assumptions and the class imbalance of the original dataset, Recall (ratio of correctly identified conflict spikes to the total number of true spikes) and Cohen Kappa Score (overall agreement between predictions and true labels adjusted for class imbalance) seem to be most relevant metrics. Two models using country features seem to perform better on these two metrics.
